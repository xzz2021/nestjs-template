...
async function initJobWithRetry(jobService: JobService, logger: Logger, maxRetries = 3): Promise<void> {
  ...
  while (retryCount < maxRetries) {
    try {
      console.info(`开始初始化定时任务队列...（第${retryCount + 1}次尝试）`);

      // 如果不是第一次尝试，等待一段时间让数据库连接更稳定
      if (retryCount > 0) {
        const waitTime = 1000 * (retryCount + 1); // 递增等待时间
        console.info(`等待${waitTime}ms后重试...`);
        await new Promise(resolve => setTimeout(resolve, waitTime));
      }

      // 执行任务队列初始化（处理未执行任务、重启必要的定时任务）
      await jobService.initJob();
      console.info('定时任务队列初始化成功');
      return; // 成功则退出
    } catch (error) {
      retryCount++;
      console.error(`定时任务队列初始化失败（第${retryCount}次尝试）`, {
        error: error.message,
        stack: error.stack,
        retryCount,
        maxRetries,
      ...
      if (retryCount >= maxRetries) {
        console.error('定时任务队列初始化最终失败，已达到最大重试次数，应用启动将被阻止');
        throw new Error(`定时任务队列初始化失败，重试${maxRetries}次后仍然失败: ${error.message}`);
      }

      // 继续下一次重试
      console.warn(`将在短暂等待后进行第${retryCount + 1}次重试...`);
    }
  }
}
