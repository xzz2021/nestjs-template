...
import { AddJobDto, AddJobLogDto, JobRunDto, UpdateJobDto } from './dto/req-job.dto';
...
export class JobService {
  ...
  async initJob() {
    ...
    /* 删除所有重复作业 */
    await Promise.all(jobSchedulers.map(item => this.jobQueue.removeJobScheduler(item.key)));
    /* 查询数据库中所有启动的任务 */
  ...
  async analysisinvokeTarget(job: AddJobDto) {
    ...
    if (splitArr.length != 2) throw new Error('调用方法格式错误');
    const serviceName = splitArr[0];
    if (!(splitArr[1].includes('(') && splitArr[1].includes(')'))) throw new Error('调用方法格式错误');
    const funName = splitArr[1].match(/(\S*)\(/)?.[1];
    if (!funName) throw new Error('调用方法格式错误');
    ...
    try {
      ...
      if (!service || !(funName in service)) {
        throw new BadRequestException('调用方法未找到');
      }
    } catch (error) {
      throw new BadRequestException('调用方法未找到');
    }
